        ...
        <div class="row">
            <div class="col-sm-4 col-md-3 vh-100 bg-light border-end">
                <div class="mt-4 mb-4 text-center">
                    ...
                    <div class="text-center mt-3 mb-3">
                        <button type="button" class="btn btn-secondary btn-sm" id="setting_button">Paramettre</button>
                        <button type="button" class="btn btn-danger btn-sm" id="logout_button">Deconnexion</button>
                    </div>
                </div>
                <hr class="bg-secondary border-2 border-top border-secondary">
                <div class="mt-4 mb-4 overflow-auto">
                    <h6 class="border-bottom pb-2 mb-0">Membre connecter</h6>
            ...
            <div class="col-sm-4 col-md-3 vh-100 bg-light border-start">
                <div class="pt-4 pb-4 h-50 overflow-auto">
                    <h6 class="mb-3">Recherche des membres</h6>
                    <input type="text" name="search_people" id="search_people" class="form-control" placeholder="Ecrire un nom..." autocomplete="off" />
...
function send_request(uc)
{
    ...
    fetch('backend/chat_request.php', {
        ...
        $('#'+uc+'').innerHTML = 'Invitation envoyer';
...
function load_chat_request()
{
    ...
    fetch('backend/chat_request.php', {
        ...
        {
            html += '<strong class="text-gray-dark">Aucune notification</strong>';
        }
...
function load_chat_connected_people()
{
    ...
    fetch('backend/chat_request.php', {
        ...
        {
            html += '<strong class="text-gray-dark">Aucune personne trouver</strong>';
        }
...
{
    var html = '<div class="row vh-100 align-items-center justify-content-center text-muted fw-bold">Selectionner une personne pour le chat</div>';
...
{
    reset_chat_area();
    var html = '<div id="chat_msg_area" class="pt-2 pb-2" style="height: 88vh;">';
    html += '<div class="card h-100">';
    html += '<div class="card-header"><div class="row"><div class="col" id="chat_user_data"></div><div class="col"><button type="button" class="btn btn-danger btn-sm float-end" id="close_chat">Fermer</button></div></div></div>';
    ...
    html += '<textarea rows="4" name="type_chat_message" id="type_chat_message" class="form-control" placeholder="Taper votre message..." aria-label="Type your message here..." aria-describedby="button-addon2"></textarea>';
    html += '<button class="btn btn-success" type="button" id="button-addon2">Envoyer</button>';
...
function setting_page()
{
    ...
    fetch('backend/fetch_user_data.php').then(function(response){
        ...
        html += '<form class="p-4 p-md-5 border rounded-3 bg-light" id="setting" method="POST" enctype="multipart/form-data" autocomplete="off"><button type="button" class="btn-close float-end" id="close_setting_page" aria-label="Close"></button>';
        html += '<h1 class="display-6 fw-bold mb-4 text-center">Paramettre</h1>';
        html += '<div class="row">';
        html += '<div class="col-md-6">';
        html += '<div class="form-floating mb-3">';
        html += '<input type="text" class="form-control" id="user_first_name" placeholder="Nom" name="user_first_name" required autocomplete="off" value="'+responseData.ufn+'">';
        html += '<label for="user_first_name">Nom</label>';
        html += '</div>';
        html += '</div>';
        html += '<div class="col-md-6">';
        html += '<div class="form-floating mb-3">';
