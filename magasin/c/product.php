				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								<label class="control-label">Matricule du produit</label>
					...
					<div class="card-body">
						...
						<div class="form-group">
							<label class="control-label">Unite de mesure</label>
						...
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										<p>Matricule : <b><?php echo $row['sku'] ?></b></p>
										<p><small>Categorie : <b><?php echo $cat ?></b></small></p>
										<p><small>Nom : <b><?php echo $row['name'] ?></b><sup><?php echo $row['measurement'] ?></sup></small></p>
										<p><small>Description : <b><?php echo $row['description'] ?></b></small></p>
									<!-- 	<?php if($row['prescription'] == 1): ?>
											<span class="badge badge-warning">Vous devez ajouter la description</span>
										<?php endif; ?> -->
									</td>
									<td>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_product" type="button" data-id="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-sku="<?php echo $row['sku'] ?>" data-category_id="<?php echo $row['category_id'] ?>" data-description="<?php echo $row['description'] ?>" data-price="<?php echo $row['price'] ?>" data-type_id="<?php echo $row['type_id'] ?>" data-price="<?php echo $row['price'] ?>" data-measurement="<?php echo $row['measurement'] ?>" data-price="<?php echo $row['price'] ?>" >Modifier</button>
										<button class="btn btn-sm btn-danger delete_product" type="button" data-id="<?php echo $row['id'] ?>">Supprimer</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('.delete_product').click(function(){
		_conf("Etes vous sur de vouloir supprimer?","delete_product",[$(this).attr('data-id')])
	})
