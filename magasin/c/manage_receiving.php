			...
			<div class="card-body">
				<form action="" id="manage-receiving">
					...
					<div class="col-md-12">
								...
								<tbody>
									...
									?>
										<tr class="item-row">
											...
											<td>
												<input type="number" step="any" name="price[]" value="<?php echo $row['price'] ?>" class="text-right">
											</td>
											<td>
						...
						<div class="row">
							<div class="col-lg-12">
								<button class="btn btn-primary col-sm-3 btn-sm btn-block float-right .col-md-3">Enregistrer</button>
							</div>
						</div>
					</div>
				</form>
...
<div id="tr_clone">
	...
	<tr class="item-row">
		...
		<td>
			<input type="number" step="any" name="price[]" value="" class="text-right">
		</td>
		<td>
			<p class="amount text-right"></p>
		</td>
...
<script>
	...
	$('#add_list').click(function(){
		...
		var product = $('#product').val(),
			...
			if(product == '' || qty == '' || price ==''){
				alert_toast("Completer tout les champs",'danger')
				return false;
			}
	...
	$('#manage-receiving').submit(function(e){
		...
		if($("#list .item-row").length <= 0){
			alert_toast("Inserer une donnee.",'danger');
			end_load();
			return false;
		}
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Donnees inserer avec success",'success')
					setTimeout(function(){
