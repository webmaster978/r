					...
					<div class="col-md-12">
						<div class="row mb-3">
								<div class="col-md-4">
									...
									<small><a href="javascript:void(0)" id="search_prod">Chercher le detail des produits.</a></small>
								</div>
								<div class="col-md-2">
									...
									?>
										<tr class="item-row">
											...
											<td>
												...
												<p class="pname">Nom: <b><?php echo $row['name'] ?> <sup><?php echo $row['measurement'] ?></sup></b></p>
						...
						<div class="row">
							<div class="col-lg-12">
								<button class="btn btn-primary col-sm-3 btn-sm btn-block float-right ">Enregistrer</button>
							</div>
						</div>
					</div>
				</form>
...
<div id="tr_clone">
	...
	<tr class="item-row">
		...
		<td>
			...
			<p class="pname">Nom: <b>produits</b></p>
...
<script>
	...
	$('#pay').click(function(){
		if($("#list .item-row").length <= 0){
			alert_toast("Inserer un element.",'danger');
			end_load();
			return false;
		}
	...
	$('#search_prod').click(function(){
		uni_modal("Trouver un produit.","find_product.php","large")
	})
	...
	$('#add_list').click(function(){
		...
		var product = $('#product').val(),
			...
			if($('#list').find('tr[data-id="'+product+'"]').length > 0){
				alert_toast("Produit deja sur la liste",'danger')
				return false;
			}

			if(product == '' || qty == ''){
				alert_toast("Completer les champs vide",'danger')
				return false;
			}
	...
	$('#manage-expired').submit(function(e){
		...
		start_load()
		if($("#list .item-row").length <= 0){
			alert_toast("Inserer aumoin un produit.",'danger');
			end_load();
			return false;
		}
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					alert_toast("Donnees envoyer avec success",'success')
